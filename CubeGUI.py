...
solveWithAStar: takes the curent cube and solves using a*, no pruning or simplifying is done
A* works until it runs out of memory at 7. Has done 6
Not IDA* '''
...
def main():
    ...
    while stop!=True:
        p = win.getMouse()
        # inside the exit box 
        if p.getX()>50 and p.getX()<100 and p.getY()>50 and p.getY()<100:
            win.close()
            stop=True


        ### new addition
        elif p.getX()>575 and p.getX()<675 and p.getY()>50 and p.getY()<100:
            print("called")
            solvedNode = solveWithAStar(rubik,faces,direct)
            print("path length to solve= "+str(solvedNode.getPathCost()))
            solLen = solvedNode.getPathCost()
            solutionBack=[]
            for num in range (solvedNode.getPathCost()):
                solutionBack.append(solvedNode.getActionToCur())
                solvedNode = solvedNode.getParent()
            while solLen>0:
                ### reads the solution backwards so that it can solve the cube
                act = solutionBack[solLen-1]
                print (act)
                curFace = act.split(" ")[0]
                curDir = act.split(" ")[1]
...
def solveWithAStar(rubik,faces,direct):
    ##(score,node)
    ...
    while pQueue.qsize()!=0:
        ...
        curNode = theTuple[1]
        #print(curNode)
        if curNode.getCurState().isSolved():
            return curNode
        else:
            done=[]
            for face in faces:
                for move in direct:
                    childNode = CubeNode(curNode.getCurState().copy(),curNode,face+" "+move,curNode.getPathCost()+1)
                    pQueue.put((childNode.getScore(),childNode))
    return None
...
def randomMoves(rubik, howMany,faces,direct,lefts,fronts,
    ...
    for turn in range(howMany):
        ...
        if lastFace!="":
            curSideOpp = isOpposite(faces,curFace,curDir,lastFace,lastDir)
            while (curFace==lastFace and curDir!=lastDir) or curSideOpp==True:
...
def isOpposite(faces,curFace,curDir,lastFace,lastDir):
    curSideOpp=False
    index = faces.index(lastFace)
    oppFace=""
    if index%2==0:
        oppFace=faces[index+1]
    else:
        oppFace=faces[index-1]
        
    if curFace==oppFace and curDir==lastDir:
        curSideOpp=True
    return curSideOpp
